...
const EditPlan = () => {
    ...
    // 🔴 Calcular tensión y APLICAR a cada aparejo
    ...
    // 🧩 Mantener ilustraciones previas si no se recapturaron
    const cleanBase64 = (str) =>
        typeof str === 'string' ? str.replace(/(\r\n|\n|\r|\s)/g, '') : str;

    let ilustracionGruaFinal = cleanBase64(
        route.params?.planData?.ilustracionGrua ||
        editablePlan.ilustracionGrua ||
        initialPlanData.ilustracionGrua ||
        'NoDisponible'
    );

    let ilustracionFormaFinal = cleanBase64(
        route.params?.planData?.ilustracionForma ||
        editablePlan.ilustracionForma ||
        initialPlanData.ilustracionForma ||
        'NoDisponible'
    );

    if (ilustracionGruaFinal && !ilustracionGruaFinal.startsWith('data:image'))
        ilustracionGruaFinal = `data:image/jpeg;base64,${ilustracionGruaFinal}`;
    if (ilustracionFormaFinal && !ilustracionFormaFinal.startsWith('data:image'))
        ilustracionFormaFinal = `data:image/jpeg;base64,${ilustracionFormaFinal}`;
    ...
    try {
        ...
        // 🔄 REFRESH DESPUÉS DEL PUT para asegurar imágenes correctas
        try {
        const refreshed = await fetch(getApiUrl(`setupIzaje/${editablePlan._id}`), {
        ...
        navigation.navigate('Tabs', {
            screen: 'Perfil',
            params: { refreshedPlan: refreshedJson.data || refreshedJson },
        });
        } catch (err) {
        console.warn("⚠️ No se pudieron refrescar los datos después del PUT:", err);
